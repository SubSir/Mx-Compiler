...
for testcase in test_file:
    try:
        content, input_data, output_data, exitcode = extract_input_output_exitcode(
            testcase
        )
        temp = open("test.in", "w")
        temp.write(input_data)
        temp.flush()
        commands = "./src/main.py > output.ll"
        ...
        process.terminate()
        commands = "clang-15 -m32 output.ll src/builtin.ll -o test"
        process = subprocess.Popen(
            commands,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
        ...
        print(
            testcase,
            (
                green_msg.format(msg="output")
                if stdout.strip() == output_data
                else red_msg.format(msg="output")
            ),
            (
                green_msg.format(msg="retcode")
                if process.returncode == int(exitcode.strip())
                else red_msg.format(msg="retcode")
            ),
        )

        if stdout.strip() == output_data and process.returncode == int(
            exitcode.strip()
        ):
            pass_cnt += 1
        process.terminate()
...
print(
    "\033[32mPassed Cases:",
    pass_cnt,
    f"\033[0m, \033[34mTotal Cases: {len(test_file)}\033[0m",
)
