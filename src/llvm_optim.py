...
class Mylistener3_5(llvmListener):
    ...
    def enterModule(self, ctx: llvmParser.ModuleContext):
        for i in ctx.getChildren():
            if isinstance(i, llvmParser.FunctionContext):
                ...
                if self.line_cnt > 2000:
                    if cnt > 50:
                        self.not_inline.add(i.Global_var().getText())
                else:
                    if cnt > 250:
                        self.not_inline.add(i.Global_var().getText())
                if cnt > 31:
    ...
    def enterCall(self, ctx: llvmParser.CallContext):
        if (
            ...
            and self.enter_func not in self.not_inline
        ):
            if ctx.params() != None:
                flag = False
...
def main(code: str) -> str:
    ...
    while True:
        ...
        def replace(key, imap, stream):
            ...
            stream[0] = stream[0].replace(key + " ", str(imap[key]) + " ")
            stream[0] = stream[0].replace(key + ",", str(imap[key]) + ",")
    ...
    listener.line_cnt = len(code.splitlines())
    walker.walk(listener, tree)
    inline_cnt = 0
    lines = code.splitlines()
    func_str = ""
